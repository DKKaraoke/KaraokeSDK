...
public extension KeyedDecodingContainer {
    func decode<T>(_ type: NullAcceptable<T>.Type, forKey key: KeyedDecodingContainer<K>.Key) throws -> NullAcceptable<T> where T: Decodable {
        try decodeIfPresent(NullAcceptable<T>.self, forKey: key) ?? NullAcceptable<T>(wrappedValue: nil)
    }
}
