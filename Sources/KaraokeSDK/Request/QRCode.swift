...
public struct QRCode {
    ...
    public init(code: String) throws {
        ...
        ip = [bytes[0], bytes[4], bytes[8], bytes[12]]
            .compactMap { UInt8($0, radix: 16) }.map { String(format: "%03d", $0) }.joined(separator: ".")
        serial = [bytes[9], bytes[11], bytes[13], bytes[15], bytes[1], bytes[3], bytes[5], bytes[7]]
            .joined(separator: "")
        timestamp = Int([bytes[14], bytes[10], bytes[6], bytes[2]].joined(separator: ""), radix: 16)!
        publishedTime = formatter.string(from: Date(timeIntervalSince1970: Double(timestamp)))
    }

    public var code: String {
        let ip: [String] = ip.split(separator: ".").compactMap { UInt8($0) }.compactMap { String(format: "%02X", $0) }
        let serial: [String] = serial.chunked(by: 2)
