...
open class DKKaraoke {
    public weak var delegate: DKKaraokeSessionDelegate?
    /// シングルトン
    public static let shared: DKKaraoke = .init()
    /// 接続用の認証情報
    ...
    public var contentsKinds: [DKContent] {
        switch credential?.deviceType {
            case .xg8000:
                [.設定しない, .精密採点DXG, .精密採点AI, .ランキングバトル]
            case .xg7000:
                [.設定しない, .精密採点DXG, .ランキングバトル]
            case .xg5000:
                [.設定しない, .精密採点DX, .ランキングバトル]
            case .unknown:
                []
            case .none:
                []
    ...
    /// スクレイピング用のコード
    func parseToHTML(data: Data, encoding: String.Encoding) -> HTMLDocument? {
    ...
    /// 認証用のリクエスト
    func authorize(_ request: Connect) -> AnyPublisher<Connect.Response, DKError> {
    ...
    public func update() -> AnyPublisher<Connect.Response, DKError> {
        ...
        return connect(qrCode: credential.code, cdmNo: credential.cdmNo)
    }
    ...
    public func search(songName: String, artistName: String) -> AnyPublisher<URL, DKError> {
        ...
        return session.request(request)
            ...
            .tryMap { response throws -> URL in
                guard let html = String(data: response, encoding: .utf8),
